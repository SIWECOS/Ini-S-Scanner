...
sub _checked_result {
    ...
    if (not $res->is_success) {
        if ($res->is_error)    {
            $job->app->log->critical("Could not callback to ".$http->req->url.": ".$res->message);
            return undef;
        }
        if ($res->code == 301) {
            $job->app->log->critical("Too many redirects for ".$http->req->url.": ".$res->headers->location);
            return undef;
        }
        $job->app->log->critical("Failed to callback to ".$http->req->url." for unknown reason");
        return undef;
    }
    return $res;
}
